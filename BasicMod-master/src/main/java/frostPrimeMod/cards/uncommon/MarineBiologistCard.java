...
public class MarineBiologistCard extends ChooseOneBaseCard {
    ...
    private static final CardInfo cardInfo = new CardInfo(
            "MARINE_BIOLOGIST_CARD", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public MarineBiologistCard() {
        super(cardInfo);
        this.keywords.add("frostprimethespire:themightyelf2");
    }
    public void use(AbstractPlayer p, AbstractMonster m){
        ArrayList<AbstractCard> stanceChoices = new ArrayList<>();
        stanceChoices.add(new NitrogenCycleOption(p, m));
        stanceChoices.add(new MemeCycleOption(p, m));
