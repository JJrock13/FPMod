...
public class MILFGangSlamCard extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "MILF_GANG_SLAM_CARD", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        int dmg = p.currentBlock + ((p.getPower(DexterityPower.POWER_ID) != null)? p.getPower(DexterityPower.POWER_ID).amount : 0);
        this.addToBot(new DamageAction(m, new DamageInfo(p, dmg, this.damageTypeForTurn), AbstractGameAction.AttackEffect.BLUNT_HEAVY));
